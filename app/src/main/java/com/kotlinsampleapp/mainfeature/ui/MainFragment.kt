...
class MainFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.searchResults.observe(viewLifecycleOwner, Observer { viewState ->
            when (viewState) {
                ...
                is SearchResponseViewState.Success -> displayItems(
                    viewState.products,
                    viewState.itemsQuantity
                )
                is SearchResponseViewState.LoadMore -> updateItems(viewState.products)
        ...
        recyclerViewAdapter.productSelectionListener = object : ProductSelectionListener {
            override fun onProductSelected(product: ProductViewData) {
    ...
    private fun displayItems(items: List<ProductViewData>, resultsAmount: Int) {
        if (resultsAmount == 1) {
    ...
    private fun updateItems(items: List<ProductViewData>) {
